...
birds <- read.csv('../3Bird_month_dist/Location_Bird_Prob/output/2.3.2.1_bird.csv')
...
od_migtimes <- read.csv('../4Markov_jump/virus_od_migtimes_bf3_2.3.2.1.csv')
...
#   mutate(birdloc_lab=case_when(birdloc=='dest' ~ 'destination',
#                                birdloc=='ori' ~ 'origin'))
write.csv(file='2.3.2.1_od_stat.csv',od_stat)
save(od_stat,file='bootstrap_2.3.2.1.RData')
...
ggplot(data = od_stat, aes(x=avgCorr, y=birdorder,color=birdloc_lab)) +
  ...
  theme_bw()
ggsave(filename = '2.3.2.1_od_forestpl.png',width=9,height=7)
...
ggplot()+
  ...
  labs(fill='Average\ncorrelation\ncoefficient',x='Bird order',
       y='Virus lineage movement route')
ggsave(filename = '2.3.2.1_od_rank_grids.png',width=7,height=7)
...
#   theme_bw()
# ggsave(filename = '2.3.2.1_loc_forestpl.png',width=7,height=6)
