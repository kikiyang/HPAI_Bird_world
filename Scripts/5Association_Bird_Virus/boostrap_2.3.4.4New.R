...
#   mutate(birdloc_lab=case_when(birdloc=='dest' ~ 'destination',
#                                birdloc=='ori' ~ 'origin'))
write.csv(od_stat, file='2.3.4.4New_od_stat.csv')
save(od_stat,file='bootstrap_2.3.4.4New.RData')
# 4. Plot figures
ggplot(data = od_stat, aes(x=avgCorr, y=birdorder,color=birdloc_lab)) +
  ...
  theme_bw()
ggsave(filename = '2.3.4.4New_od_forestpl.png',width=13,height=8)
...
ggplot()+
  ...
  labs(fill='Average\ncorrelation\ncoefficient',x='Bird order',
       y='Virus lineage movement route')
ggsave(filename = '2.3.4.4New_od_rank_grids.png',width=7,height=7)
# 
...
#   theme_bw()
# ggsave(filename = '2.3.4.4New_loc_forestpl.png',width=7,height=6)
